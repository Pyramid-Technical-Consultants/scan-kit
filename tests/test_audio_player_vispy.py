...
def _sample_render_channel() -> WaveformRenderChannel:
    ...
    mesh_pos = np.array([[0.0, -1.0], [1.0, 1.0]], dtype=np.float32)
    mesh_faces = np.array([[0, 1, 1]], dtype=np.uint32)
...
def test_envelope_mesh_builds_triangle_faces() -> None:
    t = np.array([0.0, 0.5, 1.0], dtype=np.float64)
    y_min = np.array([-1.0, -0.5, -0.2])
    y_max = np.array([1.0, 0.5, 0.2])
    verts, faces = _envelope_mesh(t, y_min, y_max)
...
def test_audio_waveform_scene_set_render_channels() -> None:
    ...
    with (
        patch.object(vispy.scene.cameras, "PanZoomCamera", MagicMock),
        patch.object(vispy.scene.visuals, "Mesh", MagicMock(return_value=mesh_mock)),
        ...
        mesh_mock.set_gl_state.assert_called_once()
