...
def _kmu_preview_rows():
    from scan_kit.workflows.config_tuning.auto_tuning.kmu_tune import KmuTunePreviewRow
    ...
    return [
        KmuTunePreviewRow(
            ...
            measured_mu=10.0,
            plan_mu=10.0,
            vs_primary_pct_before=0.0,
            vs_primary_pct_after=0.0,
            vs_plan_pct=0.0,
            old_kmu=2.0e-08,
            new_kmu=2.0e-08,
            scale=1.0,
        ...
        KmuTunePreviewRow(
            ...
            scale=1.02,
            will_write=True,
            max_energy_residual_pct=0.4,
        ),
    ]


def test_kmu_preview_table_header_matches_cell_alignment(qapp) -> None:
    from scan_kit.workflows.config_tuning.auto_tuning.kmu_preview_table import (
    ...
    fill_kmu_preview_table(table, _kmu_preview_rows())
    assert table.rowCount() == 2
    assert table.item(0, 0).text() == "IC_1_X"
    assert table.item(0, 8).text() == "no"
    assert table.item(1, 8).text() == "yes"
    for row in range(table.rowCount()):
        for col in range(table.columnCount()):
            item = table.item(row, col)
            if item is not None:
...
def test_config_tuning_panel_kmu_preview_returns_rows(qapp) -> None:
    panel = ConfigTuningPanel()
    config_root = _TEST_DATA / _SESSION / _SESSION / "config"
    assert panel.open_config_root(config_root)

    workflow = get_auto_tune_workflow("kmu_tuning")
    assert workflow is not None
    ...
    assert result is not None
    rows, _warnings = result
    assert rows
    assert any(row.device.startswith("IC_1") and not row.will_write for row in rows)
    assert any(row.family == "ic2" for row in rows)


def test_config_tuning_panel_kmu_apply_marks_devices_dirty(qapp) -> None:
    panel = ConfigTuningPanel()
    config_root = _TEST_DATA / _SESSION / _SESSION / "config"
    assert panel.open_config_root(config_root)
    ...
    assert result is not None
    assert result.success
    assert result.kmu is not None
    assert result.kmu.devices_updated > 0
